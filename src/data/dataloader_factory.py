...
class DataBuilder:
    ...
    def _generate_grid(self, grid_type: GridType, image_path: Path, wsa_path: Optional[str] = None,):
        return ImageGridFeature.init().get_full_grid(
            image_path=str(image_path),
            patch_size={"pixel": (512, 512)},
            overlap=256,
            percentage_tissue_in_tile=0.2,
        ) if grid_type == GridType.FULL else ImageGridFeature.init().get_annotation_grid(
            image_path=str(image_path),
            wsa_path=wsa_path,
